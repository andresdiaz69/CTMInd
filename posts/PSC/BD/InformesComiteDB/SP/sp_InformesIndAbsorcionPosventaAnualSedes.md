# Stored Procedure: sp_InformesIndAbsorcionPosventaAnualSedes
...
---- =============================================
CREATE PROCEDURE [dbo].[sp_InformesIndAbsorcionPosventaAnualSedes]
...
BEGIN
	...
	if @ntiendas = 0 
	Begin
		set @misql = 'No se encontraron sedes configuradas o no tienen un orden establecido'
		RAISERROR ( @misql ,   0 ,   1 )   WITH   NOWAIT WAITFOR   DELAY   '00:00:05' ;
	End
	...
	begin
		...
		IF OBJECT_ID (N'dbo.informesIndAbsorcionPosventa', N'U') IS NOT NULL
		BEGIN
			delete from informesIndAbsorcionPosventa where NombreInforme = @NombreInforme and CodigoPresentacion = @CodigoPresentacion	and Año = @Año 
			set @misql = @misql + ' insert into informesIndAbsorcionPosventa '
		END
		...
		IF OBJECT_ID (N'dbo.informesIndAbsorcionPosventa', N'U') IS NULL
		BEGIN
			set @misql = @misql + ' into informesIndAbsorcionPosventa ' 
		END
	...
	--	set @misql = @misql + ')'
		exec (@misql)
		 ...
		 Insert into informesIndAbsorcionPosventa
			...
			from
				(Select t1.Mes
				from InformesMeses t1 left outer join informesIndAbsorcionPosventa t2 on Mes = Mes_Trimestre and CodigoPresentacion = @CodigoPresentacion and Año = @Año and NombreInforme =@NombreInforme
		...
		--SET FMTONLY ON 
		--select * from #IndAbsorcionPostventaAnualSedes
	end
END
