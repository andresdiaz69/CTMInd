# Stored Procedure: sp_InformesFormato
...
-- =============================================
CREATE PROCEDURE [dbo].[sp_InformesFormato]
AS
BEGIN
	...
	WHILE @cnt <= @nColumnas
	BEGIN
		set @cmd='UPDATE TablaPresentacion SET Anterior'+ltrim(str(@cnt))+' = Anterior'+ltrim(str(@cnt))+' / 1000000,Actual'+ltrim(str(@cnt))+' = Actual'+ltrim(str(@cnt))+' / 1000000,Presupuesto'+ltrim(str(@cnt))+' = Presupuesto'+ltrim(str(@cnt))+' / 1000000'
		--print(@cmd)
		exec (@cmd)
		set @cnt = @cnt + 1
	END
