# Stored Procedure: sp_InformesIndEficienciaAdministrativa

## Usa los objetos:
- [[InformesIndEficienciaAdministrativa]]
...
CREATE PROCEDURE [dbo].[sp_InformesIndEficienciaAdministrativa]
...
BEGIN
	...
	IF OBJECT_ID (N'dbo.InformesIndEficienciaAdministrativa', N'U') IS NOT NULL
		DELETE FROM InformesIndEficienciaAdministrativa WHERE CodigoPresentacion = @CodigoPresentacion and Año = @Año
	--IF OBJECT_ID (N'dbo.InformesIndEficienciaAdministrativa', N'U') IS NOT NULL
	--	DROP TABLE dbo.InformesIndEficienciaAdministrativa
	...
	Begin
		...
		set @misql = @misql + ' 	end Eficiencia, '
		set @misql = @misql + ' 	case when sum(case when Codigoconcepto=63 then Actual'+@c+' else 0 end /'+@Divisor+') = 0 then 0 else '
--		set @misql = @misql + ' 	case when sum(case when Codigoconcepto=63 then Actual'+@c+' else 0 end) = 0 then 0 else '
		set @misql = @misql + ' 	(sum(case when Codigoconcepto=101 then Actual'+@c+' else 0 end /'+@Divisor+') + '
		set @misql = @misql + ' 	sum(case when Codigoconcepto=268 then Actual'+@c+' else 0 end /'+@Divisor+')) /  '
		set @misql = @misql + ' 	sum(case when Codigoconcepto=63 then Actual'+@c+' else 0 end /'+@Divisor+')  '
	...
	IF OBJECT_ID (N'dbo.InformesIndEficienciaAdministrativa', N'U') IS NULL Begin
		...
		set @misql2 = @misql2 + ' into InformesIndEficienciaAdministrativa from (' + @misql + ') as Datos '
		set @misql2 = @misql2 + ' group by CodigoPresentacion,NombrePresentacion,Año,Mes,NombreMes,Sede,Orden '
	End
	Else Begin
		set @misql2 = ' insert into InformesIndEficienciaAdministrativa '
		set @misql2 = @misql2 + 'Select CodigoPresentacion,NombrePresentacion,Año,Mes,NombreMes,Sede,sum(GastosdeOperacion)GastosdeOperacion,sum(GMC)GMC,sum(GAC)GAC,sum(UtilidadBruta)UtilidadBruta,sum(Eficiencia)Eficiencia,Orden,sum(EficienciaSinGac)EficienciaSinGac '     
