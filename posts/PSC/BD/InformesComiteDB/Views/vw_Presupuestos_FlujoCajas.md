# View: vw_Presupuestos_FlujoCajas

## Usa los objetos:
- [[Presupuestos_Clases]]
- [[Presupuestos_Definitivos_FlujoCajas]]
- [[Presupuestos_Jerarquias_FlujoCajas]]
- [[vw_Presupuestos_CentrosPorLinea]]

```sql
CREATE VIEW [dbo].[vw_Presupuestos_FlujoCajas] AS

-- =============================================
-- Control de Cambios
-- 2024|10|02 - Alexis - Creado para listar el presupuesto de flujos de cajas por jerarquías y líneas, mostrando el desglose mensual del valor y el porcentaje asignado a cada concepto. Utiliza un PIVOT para agrupar los valores por meses.
-- 2024|11|11 - Alexis - Se adiciono where para excluir las jerarquias 4,5,6,7,8
-- Modulo - Presupuestos
-- =============================================
...
WITH PivotedData AS (
    ...
    (
		...
		FROM Presupuestos_Definitivos_FlujoCajas	  AS Definitivo        
		INNER JOIN Presupuestos_Jerarquias_FlujoCajas AS Jerarquias		ON Definitivo.IdJerarquia = Jerarquias.IdJerarquia
		...
		WHERE Definitivo.IdJerarquia NOT IN (4,5,6,7,8)
    ) AS SourceTable
    PIVOT
    (
        SUM(Valor) 
