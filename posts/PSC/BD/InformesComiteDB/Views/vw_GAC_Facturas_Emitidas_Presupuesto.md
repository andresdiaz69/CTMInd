# View: vw_GAC_Facturas_Emitidas_Presupuesto
...
-- Date: 17/10/2024
--Descr: Create VW para proyecto presupuesto. Generera el calculo de Facturas Emitidas para GAC.
--Se replica de las vista de la base DBMLC_0190
--****************************
CREATE view [dbo].[vw_GAC_Facturas_Emitidas_Presupuesto] as 
...
FROM(
		...
		LEFT JOIN DBMLC_0190.dbo.vw_UnidadDeNegocio b ON a.IdCentros=b.CodCentro and a.IdEmpresas=b.CodEmpresa
		LEFT JOIN DBMLC_0190.dbo.Empresas c ON c.CodigoEmpresa=a.IdEmpresas
		WHERE a.IdCentros <> 137 
		and c.CodigoEmpresa  not in (22,27)
		and A.Ano_Periodo=(SELECT DATEPART(YY,GETDATE()))
		--and  A.Mes_Periodo in(10,11,12) 
		--ANd seriefactura = 'A044'
        --and numfactura = '100150'
 )A
--WHERE A.Ano_Periodo=(SELECT DATEPART(YY,GETDATE()))
--and  A.Mes_Periodo in(10,11,12) 
--ANd seriefactura = 'A044'
--and numfactura = '100150'
