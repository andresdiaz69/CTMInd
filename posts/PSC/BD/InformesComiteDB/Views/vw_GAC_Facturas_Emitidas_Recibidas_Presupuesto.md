# View: vw_GAC_Facturas_Emitidas_Recibidas_Presupuesto

## Usa los objetos:
- [[UnidadDeNegocio]]
- [[vw_GAC_Facturas_Emitidas_Presupuesto]]
- [[vw_GAC_Facturas_Recibidas_Presupuesto]]
...
-- Date: 17/10/2024
--Descr: Create VW para proyecto presupuesto. Generera el calculo de Facturas Emitidas-Recibidas para GAC.
--Se replica de las vista de la base DBMLC_0190
--****************************
CREATE VIEW [dbo].[vw_GAC_Facturas_Emitidas_Recibidas_Presupuesto] AS
       ...
       (SELECT distinct c.Ano_Periodo,   c.NombreEmpresa,               c.IdEmpresas,   
		  ...
		  FROM (
				SELECT distinct Ano_Periodo,  Mes_Periodo,  NombreEmpresa,  IdEmpresas,   nom1=NombreUnidadNegocio,  cod1=CodUnidadNegocio-- ,tip=1
				  FROM vw_GAC_Facturas_Emitidas_Presupuesto  
				 UNION ALL
				SELECT distinct Ano_Periodo,  Mes_Periodo,  NombreEmpresa,  pkfkempresas, nam1=NombreUnidadNegocio,  cod1=CodUnidadNegocio-- ,tip=2
				  FROM vw_GAC_Facturas_Recibidas_Presupuesto  				 
               )c
		  LEFT JOIN vw_GAC_Facturas_Emitidas_Presupuesto a  ON a.Ano_Periodo      =c.Ano_Periodo 
		                                       and a.Mes_Periodo      =c.Mes_Periodo 
											   and a.IdEmpresas       =c.IdEmpresas 
											   and a.CodUnidadNegocio =c.cod1
											  -- and c.tip = 1
		  LEFT JOIN vw_GAC_Facturas_Recibidas_Presupuesto b ON b.Ano_Periodo      =c.Ano_Periodo 
		                                       and b.Mes_Periodo      =c.Mes_Periodo 
											   and b.pkfkempresas     =c.IdEmpresas 
											   and b.CodUnidadNegocio =c.cod1
											  -- and c.tip = 2
		  LEFT JOIN DBMLC_0190.dbo.UnidadDeNegocio e ON e.CodUnidadNegocio=c.cod1
