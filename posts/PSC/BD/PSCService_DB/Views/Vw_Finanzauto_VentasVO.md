...
```sql
CREATE view Vw_Finanzauto_VentasVO as
select distinct vn.VIN, v.KmsActuales, vn.CodigoMarca, vn.Marca, vn.CodigoGama, vn.Gama, vn.CodigoModelo, vn.Modelo, vn.AñoModelo,
  ...
  inner join (select vin,nit, sum(valor) vr, MAX(FechaEntregaCliente) fch,IdEmpresas
                from [PSCService_DB]..spiga_VehiculosEntregados where tipo='Usados'
				 --and year(FechaEntregaCliente) >=2018 
				 --and IdEmpresas = 24 --and VIN = '1BM7200JCNH002869'
  ...
  left join [dbmlc_0190].dbo.ComisionesSpigaVO	cm on cm.codigoempresa = vn.codigoempresa
                                                  and cm.VIN = v.VIN
