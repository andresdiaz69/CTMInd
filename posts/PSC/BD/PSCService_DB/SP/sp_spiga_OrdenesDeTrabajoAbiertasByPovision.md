# Stored Procedure: sp_spiga_OrdenesDeTrabajoAbiertasByPovision
...
CREATE PROCEDURE [dbo].[sp_spiga_OrdenesDeTrabajoAbiertasByPovision]
...
SELECT	S.AñoOT
		...
		, S.Matricula
		, S.VIN
FROM	spiga_OrdenesDeTrabajoAbiertas S
WHERE	(((S.FechaFacturacion IS NULL OR S.FechaFacturacion > @FECHA) AND S.FechaEntrada <= @FECHA) AND 
		(S.Ano_Periodo = YEAR(@FECHA) AND S.Mes_Periodo = MONTH(@FECHA))) AND S.FechaCierre IS NULL
END
