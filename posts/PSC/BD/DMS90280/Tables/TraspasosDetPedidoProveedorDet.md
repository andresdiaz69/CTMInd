# Table: TraspasosDetPedidoProveedorDet

| Column Name | Data Type | Nullable |
|-------------|-----------|----------|
| PkFkEmpresas | smallint | NO |
| PkFkCentros | smallint | NO |
| PkFkAñoTraspaso | nvarchar | NO |
| PkFkSeries_Traspaso | nvarchar | NO |
| PkFkNumTraspaso | int | NO |
| PkFkTraspasoDet | int | NO |
| PkFkEmpresas_PedidoProveedor | smallint | NO |
| PkFkCentros_PedidoProveedor | smallint | NO |
| PkFkAñoPedidoProveedor | nvarchar | NO |
| PkFkSeries_PedidoProveedor | nvarchar | NO |
| PkFkPedidoProveedor | int | NO |
| PkFkPedidoProveedorDet | int | NO |
| Unidades | decimal | NO |
| UserMod | smallint | NO |
| HostMod | nvarchar | NO |
| VersionFila | tinyint | NO |
| FechaMod | datetime | NO |
