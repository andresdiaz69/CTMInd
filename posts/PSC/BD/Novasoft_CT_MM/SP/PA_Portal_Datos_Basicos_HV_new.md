# Stored Procedure: PA_Portal_Datos_Basicos_HV_new
...
--Se consultan los datos actuales de la HV del empleado
			...
			CAST(A.tel_cel AS VARCHAR(255)) AS tel_cel, CAST(P.nom_pai AS VARCHAR(255)) AS nom_pai,  
			CAST(RTRIM(A.ciu_res)+'_'+ C.nom_ciu AS VARCHAR(255)) AS  'ciu_res',CAST(RTRIM(A.dpt_res +'_'+ D.nom_dep) AS VARCHAR(255)) AS dpt_res,CAST(AE.des_est AS VARCHAR(255)) AS des_est , CAST(A.per_car AS VARCHAR(255)) AS per_car, CAST(A.e_mail AS VARCHAR(255)) AS e_mail, 
			 CAST(A.barrio AS VARCHAR(255)) AS barrio, CAST(A.ind_DecRenta AS VARCHAR(255)) AS ind_DecRenta, CAST(A.avi_emp AS VARCHAR(255)) AS avi_emp, CAST(RIGHT(A.fto_emp,5) AS VARCHAR(255)) AS fto_emp, CAST(RTRIM(A.est_civ)+'_'+ AE.des_est AS VARCHAR(255)) AS est_civ,   
...
--Se consultan los datos nuevos de la HV modificados por el empleado 
	SELECT A.cod_emp,--Campos para comparar con los nuevos
			CAST(BE.des_est AS VARCHAR(255)) AS  'des_estCiv', CAST(B.dir_res AS VARCHAR(255)) AS  'dir_res', CAST(RTRIM(B.pai_res)+'_'+  P.nom_pai AS VARCHAR(255)) AS  pai_res, 
			 CAST(RTRIM(B.ciu_res)+'_'+ C.nom_ciu AS VARCHAR(255)) AS  'ciu_res',CAST(RTRIM(A.dpt_res +'_'+ D.nom_dep) AS VARCHAR(255)) AS dpt_res,CAST(B.per_car AS VARCHAR(255)) AS  'per_car', CAST(B.e_mail AS VARCHAR(255)) AS  'e_mail', CAST(B.tel_cel AS VARCHAR(255)) AS tel_cel , CAST(B.barrio AS VARCHAR(255)) AS barrio,
			...
			CAST(B.ind_DecRenta AS VARCHAR(255)) AS  'ind_DecRenta',CAST(B.avi_emp AS VARCHAR(255)) AS 'avi_emp', B.fto_emp as 'T.fto_emp', CASE CAST(A.ind_DecRenta AS VARCHAR(255)) WHEN 0 THEN 'No aplica' WHEN '1' THEN 'Aplica' WHEN '2' THEN 'Trabajador por cuenta propia' END AS 'Declarante'
			--====================================
