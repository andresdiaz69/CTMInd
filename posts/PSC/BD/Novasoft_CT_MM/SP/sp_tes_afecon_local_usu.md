# Stored Procedure: sp_tes_afecon_local_usu
...
CREATE PROCEDURE [dbo].[sp_tes_afecon_local_usu]
	@ano_doc	CHAR(4),
	@per_doc	CHAR(2),
	@sub_tip	CHAR(5),
	@num_doc	CHAR(14),
...
BEGIN
	...
	-- INTERFACE A CNT DETALLADA SRS:2013-0167
	DECLARE @cnt_det	BIT;

	-- NUMERO CONCILIACION SRS:2013-0332
	...
	SELECT @wbco=ctactb,@nit_ban=ISNULL(nit,'0') 
	FROM tes_bancos 
	WHERE bancos=@banco;
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				SELECT cod_suc,cod_cco,cod_ter,ind_con,cod_cl1,cod_cl2,cod_cl3,cuenta_con,
				...
				GROUP BY cod_suc,cod_cco,cod_ter,ind_con,cod_cl1,cod_cl2,cod_cl3,cuenta_con,num_concil,cod_ica,cod_ret,ano_ref, per_ref, sub_ref, num_ref; 
			END;
			ELSE
			BEGIN
				DECLARE c1 CURSOR FOR
			...
			BEGIN
				...
				EXEC sp_gen_trae_tasa @wbco,@fec_tas,@tasa OUTPUT,@ind_mp;
				...
				IF @ret_doc>0
				BEGIN
					SELECT @wret=wreti FROM tes_param_cnt WHERE llave=@ind_con;
				...
				IF @iva_ret>0
				BEGIN
					SELECT @wriv=wivai FROM tes_param_cnt WHERE llave=@ind_con;
				...
				IF @val_int>0
				BEGIN
					SELECT @cta_int=cta_int 
					FROM tes_param_cnt 
					WHERE llave=@ind_con;

					EXEC sp_gen_trae_tasa @cta_int,@fec_tas,@tasa OUTPUT,@ind_mp;
				...
				-- CONTRAPARTIDA
				EXEC sp_gen_trae_tasa @wcon,@fec_tas,@tasa OUTPUT,@ind_mp;
				...
				BEGIN
					...
					SELECT b.cod_cta,a.valor,ROW_NUMBER() OVER(ORDER BY recno) 
					...
					BEGIN
						...
						EXEC sp_gen_trae_tasa @cta_imp,@fec_tas,@tasa OUTPUT,@ind_mp;
			...
			BEGIN
				...
				BEGIN
					...
					SELECT cod_suc,cod_cco,cod_ter,ind_con,cod_cl1,cod_cl2,cod_cl3,cuenta_con,num_che,numche,
							SUM(net_doc),SUM(tot_doc),SUM(des_doc),SUM(ret_doc),SUM(ica_doc),SUM(iva_ret),SUM(may_val),SUM(men_val),SUM(bas_doc),num_concil,cod_ret,cod_ica
					FROM tes_cuedoc
					WHERE ano_doc=RTRIM(@ano_doc) 
						AND per_doc=RTRIM(@per_doc) 
						AND sub_tip=RTRIM(@sub_tip) 
					...
					GROUP BY cod_suc,cod_cco,cod_ter,ind_con,cod_cl1,cod_cl2,cod_cl3,cuenta_con,num_che,numche,num_concil,cod_ret,cod_ica;
				END;
				ELSE
				BEGIN
					DECLARE cur_cpf CURSOR FOR
					SELECT cod_suc,cod_cco,cod_ter,ind_con,cod_cl1,cod_cl2,cod_cl3,cuenta_con,num_che,numche,
							net_doc,tot_doc,des_doc,ret_doc,ica_doc,iva_ret,may_val,men_val,bas_doc,num_concil,cod_ret,cod_ica
					FROM tes_cuedoc
					WHERE ano_doc=RTRIM(@ano_doc) 
						AND per_doc=RTRIM(@per_doc) 
						AND sub_tip=RTRIM(@sub_tip) 
				...
				FETCH NEXT FROM cur_cpf INTO @cod_suc,@cod_cco,@cod_ter,@ind_con,@cod_cl1,@cod_cl2,@cod_cl3,@wcon,@num_che,@numche,@net_doc,@tot_doc,@des_doc,@ret_doc,
														@ica_doc,@iva_ret,@may_val,@men_val,@bas_doc,@num_concil,@cod_ret,@cod_ica;
				WHILE @@FETCH_STATUS<>-1
				BEGIN
					...
					INSERT INTO @tinfon (ano_doc,per_doc,sub_tip,tip_doc,num_doc,reg_doc,fch_doc,cod_cta,cod_suc,cod_cco,cod_cl1,
									cod_cl2,cod_cl3,cod_ter,deb_mov,cre_mov,des_mov,num_che,ind_cos,ind_tra,
									ind_mr,fec_tas,tasa,bas_mov,ind_tas,num_concil,ind_adc)
					VALUES (@ano_doc,@per_doc,@sub_tip,@tip_doc,@num_doc,CONVERT(CHAR,@wreg),@fch_doc,@wddc,@cod_suc,CASE @i_cco WHEN 1 THEN @cod_cco ELSE '0' END,CASE @i_cl1 WHEN 1 THEN @cod_cl1 ELSE '0' END,
					...
					INSERT INTO @tinfon (ano_doc,per_doc,sub_tip,tip_doc,num_doc,reg_doc,fch_doc,cod_cta,cod_suc,cod_cco,cod_cl1,
									cod_cl2,cod_cl3,cod_ter,deb_mov,cre_mov,des_mov,num_che,ind_cos,ind_tra,
									ind_mr,fec_tas,tasa,bas_mov,ind_tas,num_concil,ind_adc)
					VALUES (@ano_doc,@per_doc,@sub_tip,@tip_doc,@num_doc,CONVERT(CHAR,@wreg),@fch_doc,@wdcc,@cod_suc,CASE @i_cco WHEN 1 THEN @cod_cco ELSE '0' END,CASE @i_cl1 WHEN 1 THEN @cod_cl1 ELSE '0' END,
					...
					FETCH NEXT FROM cur_cpf INTO @cod_suc,@cod_cco,@cod_ter,@ind_con,@cod_cl1,@cod_cl2,@cod_cl3,@wcon,@num_che,@numche,@net_doc,@tot_doc,@des_doc,
															@ret_doc,@ica_doc,@iva_ret,@may_val,@men_val,@bas_doc,@num_concil,@cod_ret,@cod_ica;
				END;
				CLOSE cur_cpf;
				DEALLOCATE cur_cpf;
			END;
		...
		BEGIN
			...
			BEGIN
				...
				SELECT cod_suc,cod_cco,cod_ter,ind_con,cod_cl1,cod_cl2,cod_cl3,cuenta_con,SUM(net_doc),SUM(tot_doc),SUM(des_doc),SUM(ret_doc),
						SUM(ica_doc),SUM(iva_ret),SUM(may_val),SUM(men_val),SUM(bas_doc),SUM(tot_imp),num_concil,SUM(base_iva_aiu),SUM(val_int)
				...
				WHERE ano_doc=RTRIM(@ano_doc) 
					...
					AND sub_tip=RTRIM(@sub_tip) 
					AND num_doc=RTRIM(@num_doc)
				GROUP BY cod_suc,cod_cco,cod_ter,ind_con,cod_cl1,cod_cl2,cod_cl3,cuenta_con,num_concil,ano_ref, per_ref, sub_ref, num_ref;
			END;
			ELSE
			BEGIN
				DECLARE cCre1 CURSOR FOR
				SELECT cod_suc,cod_cco,cod_ter,ind_con,cod_cl1,cod_cl2,cod_cl3,cuenta_con,net_doc,tot_doc,des_doc,ret_doc,
			...
			BEGIN
				...
				IF @ret_doc>0
				BEGIN
					SELECT @wret=wretn FROM tes_param_cnt WHERE llave=@ind_con;
					...
					INSERT INTO @tinfon (ano_doc,per_doc,sub_tip,tip_doc,num_doc,reg_doc,fch_doc,cod_cta,cod_suc,
									cod_cco,cod_cl1,cod_cl2,
									cod_cl3,cod_ter,deb_mov,cre_mov,des_mov,num_che,ind_cos,ind_tra,ind_mr,fec_tas,tasa,
									bas_mov,ind_tas,num_concil,ind_adc)
					VALUES (@ano_doc,@per_doc,@sub_tip,@tip_doc,@num_doc,CONVERT(CHAR,@wreg),@fch_doc,@wret,@cod_suc,
								CASE @i_cco WHEN 1 THEN @cod_cco ELSE '0' END,CASE @i_cl1 WHEN 1 THEN @cod_cl1 ELSE '0' END,CASE @i_cl2 WHEN 1 THEN @cod_cl2 ELSE '0' END,
								CASE @i_cl3 WHEN 1 THEN @cod_cl3 ELSE '0' END,CASE @i_ter WHEN 1 THEN '0' ELSE @cod_ter END,0,@ret_doc,@des_mov,@num_che,'','',@ind_mp,@fec_tas,@tasa,
								CASE @i_bas WHEN 1 THEN @base_cnt ELSE 0 END,@ind_tas,@num_concil,0);
					SET @wreg=@wreg+1;
				END;
				...
				BEGIN
					...
					VALUES (@ano_doc,@per_doc,@sub_tip,@tip_doc,@num_doc,CONVERT(CHAR,@wreg),@fch_doc,@wriv,@cod_suc,CASE @i_cco WHEN 1 THEN @cod_cco ELSE '0' END,
								...
								CASE @i_ter WHEN 1 THEN '0' ELSE @cod_ter END,0,@iva_ret,@des_mov,@num_che,'','',@ind_mp,@fec_tas,@tasa,CASE @i_bas WHEN 1 THEN @base_cnt ELSE 0 END
								,@ind_tas,@num_concil,0);
					SET @wreg=@wreg+1;
				END;
				...
				BEGIN
					...
					VALUES (@ano_doc,@per_doc,@sub_tip,@tip_doc,@num_doc,CONVERT(CHAR,@wreg),@fch_doc,@wmay,@cod_suc,CASE @i_cco WHEN 1 THEN @cod_cco ELSE '0' END,
								CASE @i_cl1 WHEN 1 THEN @cod_cl1 ELSE '0' END,CASE @i_cl2 WHEN 1 THEN @cod_cl2 ELSE '0' END,CASE @i_cl3 WHEN 1 THEN @cod_cl3 ELSE '0' END,
								CASE @i_ter WHEN 1 THEN '0' ELSE @cod_ter END,@may_val,0,@des_mov,@num_che,'','',@ind_mp,@fec_tas,@tasa,CASE @i_bas WHEN 1 THEN @bas_doc ELSE 0 END
								,@ind_tas,@num_concil,0);
					SET @wreg=@wreg+1;
				END;
				...
				BEGIN
					...
					VALUES (@ano_doc,@per_doc,@sub_tip,@tip_doc,@num_doc,CONVERT(CHAR,@wreg),@fch_doc,@wmen,@cod_suc,CASE @i_cco WHEN 1 THEN @cod_cco ELSE '0' END,
								CASE @i_cl1 WHEN 1 THEN @cod_cl1 ELSE '0' END,CASE @i_cl2 WHEN 1 THEN @cod_cl2 ELSE '0' END,CASE @i_cl3 WHEN 1 THEN @cod_cl3 ELSE '0' END,
								CASE @i_ter WHEN 1 THEN '0' ELSE @cod_ter END,0,@men_val,@des_mov,@num_che,'','',@ind_mp,@fec_tas,@tasa,CASE @i_bas WHEN 1 THEN @bas_doc ELSE 0 END
								,@ind_tas,@num_concil,0);
					SET @wreg=@wreg+1;
				END;
				...
				BEGIN
					...
					VALUES (@ano_doc,@per_doc,@sub_tip,@tip_doc,@num_doc,CONVERT(CHAR,@wreg),@fch_doc,@wica,@cod_suc,CASE @i_cco WHEN 1 THEN @cod_cco ELSE '0' END,
								CASE @i_cl1 WHEN 1 THEN @cod_cl1 ELSE '0' END,CASE @i_cl2 WHEN 1 THEN @cod_cl2 ELSE '0' END,CASE @i_cl3 WHEN 1 THEN @cod_cl3 ELSE '0' END,
								CASE @i_ter WHEN 1 THEN '0' ELSE @cod_ter END,0,@ica_doc,@des_mov,@num_che,'','',@ind_mp,@fec_tas,@tasa,CASE @i_bas WHEN 1 THEN @base_cnt ELSE 0 END
								,@ind_tas,@num_concil,0);
					SET @wreg=@wreg+1;
				END;
				...
				BEGIN
					...
					VALUES (@ano_doc,@per_doc,@sub_tip,@tip_doc,@num_doc,CONVERT(CHAR,@wreg),@fch_doc,@wdes,@cod_suc,CASE @i_cco WHEN 1 THEN @cod_cco ELSE '0' END,
								CASE @i_cl1 WHEN 1 THEN @cod_cl1 ELSE '0' END,CASE @i_cl2 WHEN 1 THEN @cod_cl2 ELSE '0' END,CASE @i_cl3 WHEN 1 THEN @cod_cl3 ELSE '0' END,
								CASE @i_ter WHEN 1 THEN '0' ELSE @cod_ter END,0,@des_doc,@des_mov,@num_che,'','',@ind_mp,@fec_tas,@tasa,CASE @i_bas WHEN 1 THEN @bas_doc ELSE 0 END
								,@ind_tas,@num_concil,0);
					SET @wreg=@wreg+1;
				END;
				...
				BEGIN
					...
					VALUES (@ano_doc,@per_doc,@sub_tip,@tip_doc,@num_doc,CONVERT(CHAR,@wreg),@fch_doc,@cta_int,@cod_suc,CASE @i_cco WHEN 1 THEN @cod_cco ELSE '0' END,
								CASE @i_cl1 WHEN 1 THEN @cod_cl1 ELSE '0' END,CASE @i_cl2 WHEN 1 THEN @cod_cl2 ELSE '0' END,CASE @i_cl3 WHEN 1 THEN @cod_cl3 ELSE '0' END,
								CASE @i_ter WHEN 1 THEN '0' ELSE @cod_ter END,@val_int,0,@des_mov,@num_che,'','',@ind_mp,@fec_tas,@tasa,CASE @i_bas WHEN 1 THEN @bas_doc ELSE 0 END
								,@ind_tas,@num_concil,0);
					SET @wreg=@wreg+1;
				END;	
				...
				VALUES (@ano_doc,@per_doc,@sub_tip,@tip_doc,@num_doc,CONVERT(CHAR,@wreg),@fch_doc,@wcon,@cod_suc,CASE @i_cco WHEN 1 THEN @cod_cco ELSE '0' END,
							CASE @i_cl1 WHEN 1 THEN @cod_cl1 ELSE '0' END,CASE @i_cl2 WHEN 1 THEN @cod_cl2 ELSE '0' END,CASE @i_cl3 WHEN 1 THEN @cod_cl3 ELSE '0' END,
							CASE @i_ter WHEN 1 THEN '0' ELSE @cod_ter END,@tot_doc,0,@des_mov,@num_che,'','',@ind_mp,@fec_tas,@tasa,CASE @i_bas WHEN 1 THEN @bas_doc ELSE 0 END
							,@ind_tas,@num_concil,0);
				SET @wreg=@wreg+1;
			...
			BEGIN
				...
				BEGIN
					...
					WHERE ano_doc=RTRIM(@ano_doc) 
						...
						AND num_che IS NOT NULL  
						AND numche IS NOT NULL 
					GROUP BY cod_suc,cod_cco,cod_ter,ind_con,cod_cl1,cod_cl2,cod_cl3,cuenta_con,num_che,numche,num_concil;
				END;
				ELSE
				BEGIN
					DECLARE cur_cpf CURSOR FOR
				...
				BEGIN
					...
					INSERT INTO @tinfon (ano_doc,per_doc,sub_tip,tip_doc,num_doc,reg_doc,fch_doc,cod_cta,cod_suc,cod_cco,
									cod_cl1,	cod_cl2,cod_cl3,
									cod_ter,deb_mov,cre_mov,des_mov,num_che,ind_cos,ind_tra,ind_mr,fec_tas,tasa,bas_mov,ind_tas,num_concil,ind_adc)
					VALUES (@ano_doc,@per_doc,@sub_tip,@tip_doc,@num_doc,CONVERT(CHAR,@wreg),@fch_doc,@wddc,@cod_suc,CASE @i_cco WHEN 1 THEN @cod_cco ELSE '0' END,
								CASE @i_cl1 WHEN 1 THEN @cod_cl1 ELSE '0' END,CASE @i_cl2 WHEN 1 THEN @cod_cl2 ELSE '0' END,CASE @i_cl3 WHEN 1 THEN @cod_cl3 ELSE '0' END,
								CASE @i_ter WHEN 1 THEN '0' ELSE @cod_ter END,@net_doc,0,@des_mov,@num_che,'','',@ind_mp,@fec_tas,@tasa,CASE @i_bas WHEN 1 THEN @bas_doc ELSE 0 END,@ind_tas,@num_concil,1);
					...
					INSERT INTO @tinfon (ano_doc,per_doc,sub_tip,tip_doc,num_doc,reg_doc,fch_doc,cod_cta,cod_suc,cod_cco,
									cod_cl1,	cod_cl2,cod_cl3,
									cod_ter,deb_mov,cre_mov,des_mov,num_che,ind_cos,ind_tra,ind_mr,fec_tas,tasa,bas_mov,ind_tas,num_concil,ind_adc)
					VALUES (@ano_doc,@per_doc,@sub_tip,@tip_doc,@num_doc,CONVERT(CHAR,@wreg),@fch_doc,@wdcc,@cod_suc,CASE @i_cco WHEN 1 THEN @cod_cco ELSE '0' END,
								CASE @i_cl1 WHEN 1 THEN @cod_cl1 ELSE '0' END,CASE @i_cl2 WHEN 1 THEN @cod_cl2 ELSE '0' END,CASE @i_cl3 WHEN 1 THEN @cod_cl3 ELSE '0' END,
								CASE @i_ter WHEN 1 THEN '0' ELSE @cod_ter END,0,@net_doc,@des_mov,@num_che,'','',@ind_mp,@fec_tas,@tasa,CASE @i_bas WHEN 1 THEN @bas_doc ELSE 0 END,@ind_tas,@num_concil,1);
		...
		BEGIN
			...
			BEGIN
				...
				WHERE ano_doc=RTRIM(@ano_doc) 
					...
					AND sub_tip=RTRIM(@sub_tip) 
					AND num_doc=RTRIM(@num_doc)
				GROUP BY cod_suc,cod_cco,cod_ter,ind_con,cod_cl1,cod_cl2,cod_cl3,cuenta_con,num_concil;
			END;
			ELSE
			BEGIN
				DECLARE cur_cpf CURSOR FOR
			...
			BEGIN
				...
				INSERT INTO @tinfon (ano_doc,per_doc,sub_tip,tip_doc,num_doc,reg_doc,fch_doc,cod_cta,cod_suc,cod_cco,
								cod_cl1,cod_cl2,cod_cl3,
								cod_ter,deb_mov,cre_mov,des_mov,num_che,ind_cos,ind_tra,ind_mr,fec_tas,tasa,ind_tas,num_concil,ind_adc)
				VALUES (@ano_doc,@per_doc,@sub_tip,@tip_doc,@num_doc,CONVERT(CHAR,@wreg),@fch_doc,@wddc,@cod_suc,CASE @i_cco WHEN 1 THEN @cod_cco ELSE '0' END,
							CASE @i_cl1 WHEN 1 THEN @cod_cl1 ELSE '0' END,CASE @i_cl2 WHEN 1 THEN @cod_cl2 ELSE '0' END,CASE @i_cl3 WHEN 1 THEN @cod_cl3 ELSE '0' END,
							CASE @i_ter WHEN 1 THEN '0' ELSE @cod_ter END,@tot_doc,0,@des_mov,@num_che,'','',	@ind_mp,@fec_tas,@tasa,@ind_tas,@num_concil,1);
				...
				VALUES (@ano_doc,@per_doc,@sub_tip,@tip_doc,@num_doc,CONVERT(CHAR,@wreg),@fch_doc,@wdcc,@cod_suc,CASE @i_cco WHEN 1 THEN @cod_cco ELSE '0' END,
							CASE @i_cl1 WHEN 1 THEN @cod_cl1 ELSE '0' END,CASE @i_cl2 WHEN 1 THEN @cod_cl2 ELSE '0' END,CASE @i_cl3 WHEN 1 THEN @cod_cl3 ELSE '0' END,
							CASE @i_ter WHEN 1 THEN '0' ELSE @cod_ter END,0,@tot_doc,@des_mov,@num_che,'','',	@ind_mp,@fec_tas,@tasa,@ind_tas,@num_concil,1);
				SET @wreg=@wreg+1;
		...
		IF @tip_doc = '070'
		BEGIN
			-- SE AGREGA EL NUMERO DE CHEQUE AL INSERT DE TES_INF_CON PARA QUE SE CREE EL DOCUMENTO CON LA INFORMACION
			...
			BEGIN
				...
				WHERE ano_doc=RTRIM(@ano_doc) 
					...
					AND sub_tip=RTRIM(@sub_tip) 
					AND num_doc=RTRIM(@num_doc)
				GROUP BY cod_suc,cod_cco,cod_ter,ind_con,cod_cl1,cod_cl2,cod_cl3,cuenta_con,num_che,num_concil;
			END;
			ELSE
			BEGIN
				DECLARE c1 CURSOR FOR
			...
			BEGIN
				...
				BEGIN
					...
					VALUES (@ano_doc,@per_doc,@sub_tip,@tip_doc,@num_doc,CONVERT(CHAR,CONVERT(SMALLINT,@max_reg)+1),
								...
								'REGISTRO DE CUADRE X REDONDEO',0,'','','',@ind_mp,@fec_tas,@tasa,@ind_tas,1);
				END;
				ELSE
				BEGIN
					SELECT @i_cco=ind_cco,@i_cl1=ind_cl1,@i_cl2=ind_cl2,@i_cl3=ind_cl3,@i_ter=ind_ter 
					FROM cnt_puc 
					WHERE cod_cta=@aju_cre;
		...
		INSERT INTO tes_inf_con(ano_doc,per_doc,sub_tip,tip_doc,num_doc,reg_doc,fch_doc,cod_suc,cod_cco,cod_cl1,cod_cl2,cod_cl3,cod_cta,cod_ter,
