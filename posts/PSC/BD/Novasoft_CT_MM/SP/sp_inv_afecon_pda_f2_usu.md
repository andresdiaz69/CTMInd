# Stored Procedure: sp_inv_afecon_pda_f2_usu
...
CREATE PROCEDURE [dbo].[sp_inv_afecon_pda_f2_usu]
	@ano_doc	CHAR(4),
	@per_doc	CHAR(2),
	@sub_tip	CHAR(5),
	@num_doc	CHAR(14)
...
BEGIN
	...
	SELECT @ind_tes=ISNULL(ind_tes,2) 
	FROM inv_param_cnt WHERE llave='0';

	SELECT @num_dec=ISNULL(dec_int,2) FROM gen_decapl WHERE cod_apl='INV' AND tip_mon=@ind_mp;

	INSERT INTO @docs (cod_egr,cod_suc,cod_cco,cod_ter,cod_cl1,cod_cl2,cod_cl3,provee,val_ant)
	SELECT cue.cod_egr,cab.cod_suc,cue.cod_cco,cab.cliente,cue.cod_cl1,cue.cod_cl2,cue.cod_cl3,cab.provee,SUM(cue.val_ant) 
	...
	GROUP BY cue.cod_egr,cue.item,cab.cod_suc,cue.cod_cco,cab.cliente,cue.cod_cl1,cue.cod_cl2,cue.cod_cl3,cab.provee;
	...
	BEGIN
		SELECT @cod_egr=cod_egr,@cod_suc=cod_suc,@cod_cco=cod_cco,@cod_ter=cod_ter,@cod_cl1=cod_cl1,@cod_cl2=cod_cl2,@cod_cl3=cod_cl3,@val_ant=val_ant
		FROM @docs
		WHERE recno=@conteo2;
		...
		IF @val_ant>0
		BEGIN		
			SET @obs_orc='DEVOLUCION ANT. EN PUNTO DE VENTA';
			...
			ELSE
			BEGIN
				SELECT @ptv_caj=cta_caj_nif, @cod_ter = ter_caj FROM inv_param_cnt WHERE llave='0';
			END;
