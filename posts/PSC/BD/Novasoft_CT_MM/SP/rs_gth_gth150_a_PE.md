# Stored Procedure: rs_gth_gth150_a_PE
...
-- Description:	Visita Domiciliaria Candidato - Temporales
-- =============================================

CREATE PROCEDURE [dbo].[rs_gth_gth150_a_PE]
	 @cod_cli		CHAR(15),
	 @cod_conv      VARCHAR(15),
	 @cod_resp		CHAR(12),
	 @num_req       INT,
...
BEGIN
	...
	SELECT	V.cod_cli, V.nom_cli, V.cod_conv, V.nom_conv, V.emp_resp, V.nom_resp, 
			RE.num_req, V.nom_car, RE.cod_emp, C.NOM_EMP, EEP.fec_ing, EEP.num_ent,
	...
	WHERE	V.cod_cli LIKE RTRIM(@cod_cli) AND V.cod_conv LIKE RTRIM(@cod_conv)
			...
			AND RE.cod_emp = RTRIM(@candidato) AND EEP.tip_ent IS NULL
			AND V.cod_cli IS NOT NULL AND EEP.num_ent = @num_vis AND EE.cod_ori = '11';
END
