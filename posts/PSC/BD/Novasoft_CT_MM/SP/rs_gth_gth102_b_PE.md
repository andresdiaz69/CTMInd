# Stored Procedure: rs_gth_gth102_b_PE
...
-- Description:     REFERENCIAS - PLANTA
-- =============================================

CREATE PROCEDURE [dbo].[rs_gth_gth102_b_PE]
	 @cod_cia		CHAR(3),
	 @cod_conv      VARCHAR(15),
	 @cod_resp		CHAR(12),
	 @num_req       INT,
...
BEGIN
	...
	SELECT	V.cod_cia, V.nom_cia, V.cod_conv, V.nom_conv, V.emp_resp, V.nom_resp, 
			RE.num_req, V.nom_car, RE.cod_emp, C.NOM_EMP, EEP.tip_ref, RT.des_tip, EEP.fec_ing,
	...
	INNER	JOIN GTH_RefeTipo AS RT ON EEP.tip_ref = RT.tip_ref
	WHERE	V.cod_cia LIKE RTRIM(@cod_cia) AND V.cod_conv LIKE RTRIM(@cod_conv)
			...
			AND V.cod_cli IS NULL AND EEP.tip_ref = @tip_ref AND EEP.num_ref = @num_ref;
END
