...
CREATE VIEW [dbo].[v_cnt_dian2006terceros]
AS
SELECT d.cod_for, L.val_top AS top_min, d.nit_ter, d.raz_soc, SUM(d.pri_val) AS valor1
	...
	, t.e_mail
FROM  dbo.Cnt_datosdian2006 AS d WITH(NOLOCK)
	INNER JOIN dbo.Cnt_concepdian2006 AS c WITH(NOLOCK) ON d.cod_for = c.cod_for AND d.cod_con = c.cod_con  
	INNER JOIN Cnt_literdian2006 AS L WITH(NOLOCK) ON C.cod_lit= L.cod_lit 
