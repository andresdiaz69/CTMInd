# View: v_MLC_consulta_empleados_activos
...
- [[v_MLC_fecha_ultimo_aumento]]

```sql
CREATE view [dbo].[v_MLC_consulta_empleados_activos] as
...
Codigo_Cargo,Nombre_Cargo,Codigo_Cargo_Generico,Nombre_Cargo_generico,Codigo_Cargo_Junta, Nombre_Cargo_Junta,email,Codigo_Departamento_trabajo,Nombre_Departamento_trabajo,
Codigo_Ciudad_trabajo,Nombre_Ciudad_trabajo,Codigo_Tipo_Contrato,Nombre_Tipo_Contrato,Fecha_Nacimiento,
genero,Indicador_salario_variable = case when Indicador_salario_variable = 1 then 'SI' else 'NO' end,Indicador_comisiones,
Prefijo_Cuenta_contable,Codigo_CNO,Descripcion_CNO,Codigo_clase_salario,Nombre_Clase_salario,SalarioBasico = sal_bas,fecha_Ult_Aumento,Porcentaje_Retencion,
...
from(
		...
		Codigo_Cargo_Junta=r.niv_car, Nombre_Cargo_Junta= v.des_niv,
		replace(replace(replace(replace(replace(replace(e.e_mail,char(13)+char(10)+'-',''),char(09),''),char(10),''),char(13),''),char(34),''),char(248),'')email,
		Fecha_Nacimiento=e.fec_nac,genero=case when e.sex_emp = 1 then 'F' else 'M' end,Indicador_salario_variable=e.ind_svar,Indicador_comisiones = b.cod_cl6,
		...
		b.sal_bas,a.fecha_Ult_Aumento,Porcentaje_Retencion=e.por_ret,e.e_mail_alt,e.tel_cel,e.tel_res
		from rhh_emplea							e
		...
		left join v_MLC_fecha_ultimo_aumento	a	on	a.cod_emp = e.cod_emp
		where e.cod_emp <> '0' 
		and (b.fec_ret is null or b.fec_ret > getdate())
) a	 --where cedula in ('79578100','51975078','79509714')
--1663
