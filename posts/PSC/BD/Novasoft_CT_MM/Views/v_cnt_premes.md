...
FROM cnt_presup AS acu WITH(NOLOCK)
...
SELECT acu.ano_acu, '02' AS Periodo, acu.cod_cta, puc.nom_cta, puc.niv_cta, puc.tip_cta, acu.cod_suc, 
	...
	suc.nom_suc, cco.nom_cco, cl1.nombre AS nom_cl1, cl2.nombre AS nom_cl2, cl3.nombre AS nom_cl3, puc.nat_cta
FROM cnt_presup AS acu WITH(NOLOCK)
	INNER JOIN cnt_puc puc WITH(NOLOCK) ON acu.cod_cta=puc.cod_cta
	INNER JOIN gen_sucursal suc WITH(NOLOCK) ON acu.cod_suc=suc.cod_suc
	INNER JOIN gen_ccosto cco WITH(NOLOCK) ON acu.cod_cco=cco.cod_cco
	INNER JOIN gen_clasif1 cl1 WITH(NOLOCK) ON acu.cod_cl1=cl1.codigo
	INNER JOIN gen_clasif2 cl2 WITH(NOLOCK) ON acu.cod_cl2=cl2.codigo
	INNER JOIN gen_clasif3 cl3 WITH(NOLOCK) ON acu.cod_cl3=cl3.codigo
